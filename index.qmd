...
In total, there are 189 samples (One missing - which?). A contract was set up for 100 hours:
...
Quality control was performed on raw and trimmed reads using FastQC (v0.11.8) and FastP (v0.23.2). The results for all samples were summarized using MultiQC (v). Sequencing adapters and low quality bases and reads were removed using Trimmomatic (wrapped in TrimGalore v0.6.1) using the standard parameters.
